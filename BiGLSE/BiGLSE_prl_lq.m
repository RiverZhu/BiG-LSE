function [out, dbg_opt] = BiGLSE_prl_lq(y, z0, a_theta, Init, dbg_ipt)
    ...
    for iter=1:Iter_max
        ...
        if B==inf
            ...
            % zeta = [real(Init.zeta);imag(Init.zeta)];
            zeta = 0;
    ...
    L_dy = length(omega_hat); xhat_dy = zeros(L_dy,1);
    ...
    for iter_ref = 1:Iter_ref_max
        ...
        if iter_ref>=3
            if iter_ref~=1
                dict_idx = find(abs(xhat_dy)<=1e-1);
        ...
        tau_p_dy_old = 1e3*ones(N,1);
        tau_s_dy_old = 1e3*ones(N,1);
        dhat_dy_old = zeros(L_dy,1);
        tau_x_dy = 1e3*ones(L_dy,1);
        dhat_dy = zeros(L_dy,1);
        tau_d_dy = 1e2*ones(L_dy,1);
        rhat_dy_old = zeros(L_dy,1);
        tau_r_dy_old = 1e3*ones(L_dy,1);
        ...
        omega_hat = wrapTo2Pi(omega_hat + dhat_dy/N);
        if B==1
            omega_amp = abs(c*xhat_dy);
        else
            omega_amp = abs(xhat_dy);
        end
        % for debug
    ...
    xhat_dy = xhat_dy(sort_idx);
    zhat_dy = exp(1j*n*omega_hat')/sqrt(N)*xhat_dy;
    if B==1
        NMSE_iter_final(Iter_in_max,Iter_ref_max) = norm(z0-c*zhat_dy)/norm(z0);
    else
