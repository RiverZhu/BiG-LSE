function [out, dbg_opt] = BiGLSE_seq(y, z0, Parameter, Init, dbg_ipt)
...
for iter_ot = 1:Iter_ref_max
    ...
    tau_x = 1e2*ones(L,1); xhat_damp = xhat;
    dhat = zeros(L,1); tau_d = 1e2*ones(L,1);
    a_theta = exp(1j*n*omega_hat')/sqrt(N);
    mu_x0 = 0; sigvar0 = 1e2; pivar0 = 0.5*ones(L,1); sigma_w0 = 1e2;
    rhat_old = zeros(L,1); tau_r_old = 1e2*ones(L,1);
    qhat_old = zeros(L,1); tau_q_old = 1e2*ones(L,1);
    shat = zeros(N,1); shat_old = zeros(N,1); tau_s_old = 1e2*ones(N,1);
    tau_p_old = 1e2*ones(N,1);
    dhat_old = zeros(L,1);
